...
> Nguồn: email spec khách gửi 31/07/2026. File này để paste vào đầu mỗi task/chat trong Cursor làm context chung, sau đó paste thêm phần task riêng bên dưới.

---

## 0. DESIGN TOKENS (context dùng cho MỌI task — luôn paste đầu tiên)

```
Colors:
- bg-primary: #FFFFFF (trắng)
- bg-secondary: #F7F5F2 (trắng ngà nhạt) — 2 màu nền đổi chỗ luân phiên theo từng section
- cta-bg: cam #F97316, chữ trắng
- footer-bg: navy #0B1F3A
- logo "Nhà": cam #F97316
- logo "Web": navy #0B1F3A

Typography: font tham khảo từ https://www.perpetualny.com/ (font-family: "Surt Expanded";)

Global UI behaviors:
- Card: có border + màu nền full trong card (style giống luvini.vn)
- Ảnh: hover/click → scale nhẹ lên (ref: tma.vn)
- Menu: click → sub-menu xổ ra, hover text → highlight (giống luvini.vn, đã có sẵn code mẫu)
- Phone + Zalo icon: float góc dưới phải, luôn hiện (ref: interdata.vn)
```

**Việc cần làm trước khi giao task cho AI:** chụp screenshot luvini.vn (menu + card), tma.vn (ảnh zoom khi click), interdata.vn (float icon + section quy trình 5 bước), web4s.vn (section 1 + FAQ + tin tức), vinahost.vn (CTA button style), perpetualny.com (font + card bg), msn.com/vi-vn (carousel 4 items auto next 4s).
